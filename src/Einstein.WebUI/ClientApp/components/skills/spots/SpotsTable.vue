  ...
  <b-tooltip label="Redo latest change..." position="is-bottom"><button class="button bbar-middle is-secondary" @click="redo()"><i class="fas fa-redo-alt" /></button></b-tooltip>
    ...
    <div class="columns">
      <div class="column">
        ...
        <b-pagination :total="!spots ? 0 : spots.length"
  ...
  export default {
    ...
    methods: {
      ...
      refreshCSSClasses() {
        ...
        _forEach(that.spots, function (row) {
      ...
      removeRow() {
        this.removeSpot(this.spots[this.selectedSeqno]).then(() => {
          this.refreshCSSClasses();
        });
      },
      ...
      onPageChange: function (newPage) {
        if (this.$refs.spotsTable && newPage) {
          this.$refs.spotsTable.setPage(newPage);
        }
      },
