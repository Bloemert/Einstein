  ...
  <b-tooltip label="Redo latest change..." position="is-bottom"><button class="button bbar-middle is-secondary" @click="redo()"><i class="fas fa-redo-alt" /></button></b-tooltip>
    ...
    <div class="columns">
      <div class="column">
        ...
        <b-pagination :total="!skillcategories ? 0 : skillcategories.length"
      ...
      <div class="column">
        <skillcategory-form></skillcategory-form>
      </div>
    </div>
...
<script>
  ...
  import { mapSkillCategoryActions, mapSkillCategoryFields, mapSkillCategoryMultiRowFields } from './store';
  ...
  export default {
    name: 'SkillCategoriesMasterDetail',
    ...
    methods: {
      ...mapSkillCategoryActions({
        ...
        saveData: 'saveSkillCategoryChangesToDB'
      }),
      ...
      refreshCSSClasses() {
        ...
        _forEach(that.skillcategories, function (row) {
      ...
      addRow() {
        this.addSkillCategory().then(() => {
          this.refreshCSSClasses();
        });
      },

      removeRow() {
        this.removeSkillCategory(this.skillcategories[this.selectedSeqno]).then(() => {
          this.refreshCSSClasses();
        });
      },
      ...
      onPageChange: function (newPage) {
        if (this.$refs.skillCategoriesTable && newPage) {
          this.$refs.skillCategoriesTable.setPage(newPage);
        }
      },
    ...
    computed: {
      ...
      ...mapSkillCategoryFields({
        skillcategories: 'rows',
    ...
    components: {
      'skillcategory-form': SkillCategoryForm
    }
