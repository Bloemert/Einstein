    ...
    <div class="columns">
      <div class="column">
        ...
        <v-client-table class="table table-body-scroll"
          ...
          <div slot="filter__version">
            <b-input type="search" class="form-control table-column-filter" v-model="filterVersion" @input="searchByField('version', $event)" placeholder="Filter by Version"></b-input>
          </div>
        </v-client-table>
        <b-pagination :total="!skills ? 0 : skills.length"
  ...
  export default {
      ...
      return {
        ...
        options: {
          ...
          customFilters: [{
            ...
            name: 'version',
            callback: function (row, query) {
              return row.version.indexOf(query.version) !== -1;
            }
          }
          ],
    ...
    methods: {
      ...
      refreshCSSClasses() {
        ...
        _forEach(that.skills, function (row) {
      ...
      removeRow() {
        this.removeSkill(this.skills[this.selectedSeqno]).then(() => {
          this.refreshCSSClasses();
        });
      },
      ...
      onPageChange: function (newPage) {
        if (this.$refs.skillsTable && newPage) {
          this.$refs.skillsTable.setPage(newPage);
        }
      },
