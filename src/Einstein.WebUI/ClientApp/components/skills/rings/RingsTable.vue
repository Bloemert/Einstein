  ...
  <b-tooltip label="Redo latest change..." position="is-bottom"><button class="button bbar-middle is-secondary" @click="redo()"><i class="fas fa-redo-alt" /></button></b-tooltip>
    ...
    <div class="columns">
      <div class="column">
        ...
        <b-pagination :total="!rings ? 0 : rings.length"
...
<script>
  ...
  import RingForm from './RingForm.vue';
  ...
  export default {
    ...
    methods: {
      ...
      refreshCSSClasses() {
        ...
        _forEach(that.rings, function (row) {
      ...
      removeRow() {
        this.removeRing(this.rings[this.selectedSeqno]).then(() => {
          this.refreshCSSClasses();
        });
      },
      ...
      onPageChange: function (newPage) {
        if (this.$refs.ringsTable && newPage) {
          this.$refs.ringsTable.setPage(newPage);
        }
      },
