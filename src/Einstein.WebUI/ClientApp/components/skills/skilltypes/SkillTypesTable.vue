  ...
  <b-tooltip label="Redo latest change..." position="is-bottom"><button class="button bbar-middle is-secondary" @click="redo()"><i class="fas fa-redo-alt" /></button></b-tooltip>
    ...
    <div class="columns">
      <div class="column">
        ...
        <b-pagination :total="!skilltypes ? 0 : skilltypes.length"
...
<script>
  ...
  import { mapSkillTypeActions, mapSkillTypeFields, mapSkillTypeMultiRowFields } from './store';


  import SkillTypeForm from './SkillTypeForm.vue';
  ...
  export default {
    ...
    methods: {
      ...mapSkillTypeActions({
        ...
        saveData: 'saveSkillTypeChangesToDB'
      }),
      ...
      refreshCSSClasses() {
        ...
        _forEach(that.skilltypes, function (row) {
      ...
      addRow() {
        this.addSkillType().then(() => {
          this.refreshCSSClasses();
        });
      },

      removeRow() {
        this.removeSkillType(this.skilltypes[this.selectedSeqno]).then(() => {
          this.refreshCSSClasses();
        });
      },
      ...
      onPageChange: function (newPage) {
        if (this.$refs.skilltypesTable && newPage) {
          this.$refs.skilltypesTable.setPage(newPage);
        }
      },
